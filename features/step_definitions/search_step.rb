# require_relative 'pages/search_actions'
...
When("I search for {string}") do |string|
  search_radar_input_field.send_keys(string)
  search_radar_submit.click()
end

Then("{string} related results are displayed") do |string|
    has_search_results_title = @wait.until {search_results_title[0]}
  if expect(has_search_results_title)
    expect(has_search_results_title.text).to be == 'Matching results for "'+string+'"'
  end
end
